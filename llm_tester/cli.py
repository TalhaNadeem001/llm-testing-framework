...
def batch(openai_model, gemini_model, qwen_model, deepseek_model, test_file, system_prompt_file, check_confidence, output_file):
    """Run batch intent classification tests against OpenAI, Gemini, Qwen, and DeepSeek."""
    ...
    asyncio.run(run_batch(test_cases, openai_model, gemini_model, qwen_model, system_prompt, check_confidence, output_file, deepseek_model))
...
@click.option("--output-file", default=None, help="Path to save results as a .txt file.")
def batch_execution(openai_model, gemini_model, qwen_model, deepseek_model, test_file, system_prompt_file, output_file):
    """Run batch tool-calling tests for the Execution Agent against OpenAI, Gemini, Qwen, and DeepSeek."""
    ...
    asyncio.run(run_execution_batch(test_cases, openai_model, gemini_model, system_prompt, output_file, qwen_model, deepseek_model))
...
@click.option("--output-file", default=None, help="Path to save results as a .txt file.")
def batch_composer(test_file, system_prompt_file, openai_model, gemini_model, qwen_model, deepseek_model, judge_model, output_file):
    """Run LLM-as-judge batch tests for the Composer Agent against OpenAI, Gemini, Qwen, and DeepSeek."""
    ...
    asyncio.run(run_composer_batch(test_cases, openai_model, gemini_model, system_prompt, judge_model, output_file, qwen_model, deepseek_model))
